...
class wow_event_teleporter : public CreatureScript
{
    public:

        wow_event_teleporter()
            : CreatureScript("wow_event_teleporter")
        {
        }

        bool OnGossipHello(Player* player, Creature* creature)
        {
            player->ADD_GOSSIP_ITEM(2, "Zeige mir die Events",                                  GOSSIP_SENDER_MAIN,   1);
        ...
        { 
			...
			if (action == 1)
			{
				...
				{
				player->ADD_GOSSIP_ITEM(2, "Labyrint I",                        GOSSIP_SENDER_MAIN,  10);
				player->ADD_GOSSIP_ITEM(2, "Labyrint II",                        GOSSIP_SENDER_MAIN,  11);
                player->ADD_GOSSIP_ITEM(2, "Labyrint III",                    GOSSIP_SENDER_MAIN,  12);
                player->ADD_GOSSIP_ITEM(2, "Treppe",                           GOSSIP_SENDER_MAIN,  13);
                player->ADD_GOSSIP_ITEM(2, "Labyrinth IV",                        GOSSIP_SENDER_MAIN,  14);
			...
			/*	else
				{
				player->ADD_GOSSIP_ITEM(2, "New event",                        GOSSIP_SENDER_MAIN,  16);
		...
		if (action == 9)
		{
			...
			if (action == 10)
			{								
				player->CLOSE_GOSSIP_MENU(); player->TeleportTo(1,	2461.12f,	2447.85f, 	131.415f, 4.52741f);									
			}
			if (action == 11)
			{
				player->CLOSE_GOSSIP_MENU(); player->TeleportTo(0,	-7311.88f,   1028.33f,	131.409f, 1.49336f); ;  
			}
			if (action == 12)
			{
				player->CLOSE_GOSSIP_MENU(); player->TeleportTo(0,	-7503.49f,    796.361f,	 131.407f, 6.28114f);
			}
			if (action == 13)
			{
				player->CLOSE_GOSSIP_MENU(); player->TeleportTo(1,	2910.67f,    2923.62f,	 3.55366f, 1.31152f);
			}
			if (action == 14)
			{
				player->CLOSE_GOSSIP_MENU(); player->TeleportTo(1,  -7448,   58,    4, 0.000000f);
			}
			if (action == 15)
			{
			...
			if (action == 16)
			{
				player->CLOSE_GOSSIP_MENU(); player->TeleportTo(0,	5892.241211f,	14554.584961f, 	91, 3.483906f);
			}
			if (action == 17)
			{
