...
class wow_jump_teleporter : public CreatureScript
{
    public:

        wow_jump_teleporter()
            : CreatureScript("wow_jump_teleporter")
        {
        }

        bool OnGossipHello(Player* player, Creature* creature)
        {
            player->ADD_GOSSIP_ITEM(2, "Zeig mir die Jump Events",                                  GOSSIP_SENDER_MAIN,   1);
        ...
        { 
			...
			if (action == 1)
			{
				...
				{
				player->ADD_GOSSIP_ITEM(2, "Jump Event I",                        GOSSIP_SENDER_MAIN,  10);
				player->ADD_GOSSIP_ITEM(2, "Jump Event II",                        GOSSIP_SENDER_MAIN,  11);
                player->ADD_GOSSIP_ITEM(2, "Jump Event III",                    GOSSIP_SENDER_MAIN,  12);
		...
		if (action == 9)
		{
			...
			if (action == 10)
			{								
				player->CLOSE_GOSSIP_MENU(); player->TeleportTo(0,	-14209,	753, 9, 1);									
			}
			if (action == 11)
			{
				player->CLOSE_GOSSIP_MENU(); player->TeleportTo(1,     3523,   -7333,   11, 0.000000f);  
			}
			if (action == 12)
			{
				player->CLOSE_GOSSIP_MENU(); player->TeleportTo(1,    10349,   251,    1330, 0.000000f);
			}
			if (action == 13)
			{
