...
class wow_pvp_teleporter : public CreatureScript
{
    public:

        wow_pvp_teleporter()
            : CreatureScript("wow_pvp_teleporter")
        {
        }

        bool OnGossipHello(Player* player, Creature* creature)
        {
            player->ADD_GOSSIP_ITEM(2, "RDW PvP Arena",                                  GOSSIP_SENDER_MAIN,   1);
        ...
        { 
			...
			if (player->getLevel() == 80)
			{
				...
				{
				player->ADD_GOSSIP_ITEM(2, "Und los gehts",                        GOSSIP_SENDER_MAIN,  16);
				...
				ChatHandler(player->GetSession()).PSendSysMessage("Port erst ab Level 80 Moeglich !");
				}
