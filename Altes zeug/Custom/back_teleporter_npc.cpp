...
class wow_back_teleporter : public CreatureScript
{
    public:

        wow_back_teleporter()
            : CreatureScript("wow_back_teleporter")
        {
        }

        bool OnGossipHello(Player* player, Creature* creature)
        {
            player->ADD_GOSSIP_ITEM(2, "Zur\303\274ck",                                  GOSSIP_SENDER_MAIN,   1);
        ...
        { 
			...
			if (action == 1)
			{
				...
				{
				player->ADD_GOSSIP_ITEM(2, "Im Treff gestorben",                        GOSSIP_SENDER_MAIN,  10);
			 	player->ADD_GOSSIP_ITEM(2, "Azshara Krater gestorben",                        GOSSIP_SENDER_MAIN,  11);
                player->ADD_GOSSIP_ITEM(2, "In der PvP Zone gestorben",                    GOSSIP_SENDER_MAIN,  12);
               /*  player->ADD_GOSSIP_ITEM(2, "Im Shadopan Event gestorben",                           GOSSIP_SENDER_MAIN,  13);
               player->ADD_GOSSIP_ITEM(2, "Shattrath",                        GOSSIP_SENDER_MAIN,  14);
				...
				{
				player->ADD_GOSSIP_ITEM(2, "Im Treff gestorben",                        GOSSIP_SENDER_MAIN,  10);
				player->ADD_GOSSIP_ITEM(2, "Azshara Krater gestorben",                        GOSSIP_SENDER_MAIN,  13);
                player->ADD_GOSSIP_ITEM(2, "In der PvP Zone gestorben",                    GOSSIP_SENDER_MAIN,  14);
               /*   player->ADD_GOSSIP_ITEM(2, "Im Shadopan Event gestorben",                           GOSSIP_SENDER_MAIN,  13);
              player->ADD_GOSSIP_ITEM(2, "Shattrath",                        GOSSIP_SENDER_MAIN,  14);
                player->ADD_GOSSIP_ITEM(2, "Dalaran",                          GOSSIP_SENDER_MAIN,  15);
				player->ADD_GOSSIP_ITEM(0, "Mainmenü",                        GOSSIP_SENDER_MAIN,   9);*/
		...
		if (action == 9)
		{
			...
			if (action == 10)
			{								
				player->CLOSE_GOSSIP_MENU(); player->TeleportTo(1,	7422,	-1608, 180, 1);				
				player->ResurrectPlayer(1.0f);
				player->CastSpell(player, 6962, true);
			}
			if (action == 11)
			{
				player->CLOSE_GOSSIP_MENU(); player->TeleportTo(37,     441.805695,   146.018967,   269, 0.000000f);  
				player->ResurrectPlayer(1.0f);
				player->CastSpell(player, 6962, true);
			}
			if (action == 12)
			{
				player->CLOSE_GOSSIP_MENU(); player->TeleportTo(0,	4327,	-2875,	2, 2.2);	
				player->ResurrectPlayer(1.0f);
				player->CastSpell(player, 6962, true);
			}
			if (action == 13)
			{
				player->CLOSE_GOSSIP_MENU(); player->TeleportTo(37,  151.111740, 267.140045, 274, 0.000000f);
				player->ResurrectPlayer(1.0f);
				player->CastSpell(player, 6962, true);
			}
			if (action == 14)
			...
			if (action == 16)
			{
				player->CLOSE_GOSSIP_MENU(); player->TeleportTo(0,	17422.43,	-1608.77,	1792.48861, 1);
			}
			if (action == 17)
			{
