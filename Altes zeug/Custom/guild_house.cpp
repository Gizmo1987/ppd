	/* Made by: QQRofl Not sure
       Updated by: Synth
       Script: Guild Housing
       Tested: untested in game
       Shared For: Emudevs
    */
         ...
         //   if(isInGuildArea(pPlayer))
            {
                    ...
                    pPlayer->GetSession()->SendNotification("Du hast Erfolgreich euer Gilden Haus Verkauft!", LANG_UNIVERSAL, pPlayer->GetGUID());
    ...
    void PurchaseGuildHouse(Player * pPlayer, Creature * pCreature, uint32 &Id)
    {
            if(pPlayer->HasItemCount(GUILD_TOKEN, 1))
            {
                    ...
                    pPlayer->GetSession()->SendNotification("Herzlichen Gl\303\274ckwunsch zum erwerb ihres neuen Gilden Hauses!", LANG_UNIVERSAL, pPlayer->GetGUID());
                    pPlayer->DestroyItemCount(GUILD_TOKEN, 1, true);
                    requiresGUpdate = true;
            }
            else
            {
                     pPlayer->GetSession()->SendNotification("Du braucht ein Gilden Haus Token zum kauf!", LANG_UNIVERSAL, pPlayer->GetGUID());
            }
                    pPlayer->PlayerTalkClass->SendCloseGossip();
    }
    ...
    class GuildHouse_NPC : public CreatureScript
    {
            public:
                    ...
                    void ShowHouseListings(Player * pPlayer, Creature * pCreature, unsigned int start = 0)
                    {
                            ...
                            for(unsigned int i = start; i < houses; i++)
                            {
                                    ...
                                    else
                                    {
                                            ...
                                            {
                                                    pPlayer->ADD_GOSSIP_ITEM(7, "Neste Seite", GOSSIP_SENDER_MAIN, 4353);
                    ...
                    bool OnGossipHello(Player * pPlayer, Creature * pCreature)
                    {
                            ...
                            {
                                    pPlayer->GetSession()->SendNotification("Du musst in einer Gilde Sein!");
                                    return false;
                            }
                            else if (!GuildOwnsHouse(pPlayer) && !isPlayerGuildLeader(pPlayer))
                            {
                                    pPlayer->GetSession()->SendNotification("Deine Gilde besitzt kein Haus!");
                                    return false;
                            }
                            pPlayer->ADD_GOSSIP_ITEM(4, "Gilden Haus System:", GOSSIP_SENDER_MAIN, 41);
                            pPlayer->ADD_GOSSIP_ITEM(4, "----------------------------------------------", GOSSIP_SENDER_MAIN, 41);
                            if(isPlayerGuildLeader(pPlayer))
                            {
                                    if(!GuildOwnsHouse(pPlayer))
                                            pPlayer->ADD_GOSSIP_ITEM(6, "Kauf ein Gilden Haus", GOSSIP_SENDER_MAIN, 38);
                                    else
                                            pPlayer->ADD_GOSSIP_ITEM_EXTENDED(6, "Verkauf dein Gilden Haus", GOSSIP_SENDER_MAIN, 39, "Bist du sicher das du dein Gilden Haus verkaufen moechtest?", 0, false);
                            }
                            if(GuildOwnsHouse(pPlayer))
                                    pPlayer->ADD_GOSSIP_ITEM(7, "Teleportier mich zu meinem Gilden Haus", GOSSIP_SENDER_MAIN, 40);
                            pPlayer->PlayerTalkClass->SendGossipMenu(2, pCreature->GetGUID());
                            return true;
                    }
    ...
    class Table_Token : public ItemScript
    {
            public:
                    ...
                    bool OnUse(Player* pPlayer, Item* pItem, SpellCastTargets const& /*targets*/)
                    {
                            ...
                            {
                                    pPlayer->GetSession()->SendNotification("Du musst ein Gilden Haus besitzen um dieses  Item nutzen zu koennen!");
                                    return false;
                            }
     
                            if(!isInGuildArea(pPlayer))
                            {
                                    pPlayer->GetSession()->SendNotification("Du musst in deinem Gilden Haus sein um dieses Item benutzen zu koennen!");
                                    return false;
                            }
    ...
    class Guild_Token_Vendor : public CreatureScript
    {
            public:
                    ...
                    bool OnGossipHello(Player * pPlayer, Creature * pCreature)
                    {
                            if(!isPlayerGuildLeader(pPlayer))
                            {
                                    pPlayer->GetSession()->SendNotification("Nur der Gilden Meister hatt zugang!");
                                    pPlayer->PlayerTalkClass->SendCloseGossip();
                                    return false;
                            }
                            ...
                            {
                                    pPlayer->PlayerTalkClass->ClearMenus();
                                    pPlayer->ADD_GOSSIP_ITEM(4, "Zeige mir bitte die Objecte!", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_TRADE);
                                    pPlayer->ADD_GOSSIP_ITEM(3, "Spaeter vielleicht...", GOSSIP_SENDER_MAIN, 2);
                                    pPlayer->PlayerTalkClass->SendGossipMenu(2, pCreature->GetGUID());
                                    //return true;
                            }
                            else
                            {
                                    pPlayer->GetSession()->SendNotification("Du musst ein Gilden Haus besitzen!");
                                    return false;
                            }
                            return true;
                    }
